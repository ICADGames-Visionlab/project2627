...
um clique ou um comando, sem recompilar nem mexer em código.
...
`DebugConsoleOverlay` (cenas instanciadas sob demanda) são dois jeitos de ler o mesmo registro, então uma
ação registrada nasce como botão no menu e como comando no console de uma vez só. Ninguém precisa
desregistrar: um `Callable` cujo dono já saiu da árvore some sozinho na próxima abertura do menu.

> **Estado atual:** o projeto ainda não tem gameplay, então as únicas seções são as de fábrica
> (**Sistema**, **Desempenho**, **Tela**, **Captura** e **Log**, todas descritas abaixo), que não dependem
> de nenhum sistema de jogo. Elas também servem de exemplo vivo de como registrar. A seção **Eventos** (ver
...
- `register_value(section, label, on_changed, param, getter = Callable())` — um campo solto (número, texto,
  ...
  ao remontar em vez da cópia guardada, o que resolve o mesmo desencontro do toggle.
...
**Registrar de novo com o mesmo par (seção, rótulo) substitui a entrada anterior** em vez de duplicar, o
...
O próprio `DebugMenu` registra estas cinco entradas no `_ready()`, antes de qualquer cena de jogo, e por
isso "Sistema" aparece sempre em primeiro:
...
| Sair do jogo | action, **pede confirmação** | `get_tree().quit()`, que mata a sessão. Ver ["Ação destrutiva"](#ação-destrutiva) |
...
divergem. O OSD é independente do menu: a ideia é deixá-lo ligado enquanto se joga.
...
> ou o outro lado). Para a pergunta "quem está me segurando", essa é a métrica útil. Ela só não é a mesma
> métrica que o Afterburner mostra.
>
...
um punhado de draw calls; os mesmos cem sprites com texturas soltas custam cem. O tempo de CPU e de GPU
mostra o resultado disso só depois que já está caro. A contagem mostra a causa.
...
reportavam 30 a 44 ms para quadros de 16,67 ms, e usá-los deixava "Uso CPU" grudado em 100%.
...
da esquerda fica visualmente apagada enquanto isso. É proposital. Quem procura uma ação em geral não lembra
em que seção ela está, e é essa a pergunta que o filtro responde: o modelo é o de uma command palette, não o
de um filtro por categoria.
...
O critério é reversibilidade, não gravidade aparente. "Pausar jogo" é trivial de reverter
...
- O diálogo (`ConfirmationDialog`) é único por overlay e reusado por toda entrada, então clicar em várias
  ações destrutivas em sequência não acumula conexões nem abre um diálogo por clique.
...
do console **são** as entradas do `DebugMenu`, então registrar uma ação dá as duas coisas de uma vez.
...
A linha de uso (`Uso: ...`) sai direto dos `DebugParam` declarados no registro, e não tem como ficar
...
**Esquecer o guard de debug.** `register_action()`/`register_toggle()` já não fazem nada em build de
release, mas sem o guard `if OS.has_feature("editor") or OS.is_debug_build():` no call site, o Guideline de
código `[DEBUG]` não é cumprido, e é isso que a revisão de PR cobra.

**Registrar uma ação que depende de nó de outra cena.** Guarde a referência do próprio nó dono do
`Callable`, nunca de um nó externo capturado por closure. Se o dono sair da árvore, o `Callable` deixa de
ser válido e a entrada some sozinha (ver "Callable morto" abaixo); se for o nó externo que sai, o clique
quebra sem aviso.
...
## Os textos da ferramenta não passam por `tr()`

O Guideline proíbe texto hardcoded, e a regra vale para **texto exibido ao jogador**, que tem de sair da
base de localização. O Debug Menu é a exceção deliberada: rótulo de seção, nome de ação, cabeçalho do OSD,
mensagem de erro do console e os textos das cenas de `scenes/debugs/` são escritos direto no código.

Por quê:

- **Nada disto existe em build de release.** `register_*()`, `execute_command()` e os atalhos caem fora pelo
  guard de debug, então nenhuma dessas strings chega ao jogador: traduzir custaria manutenção e teria
  alcance zero.
- **O rótulo é a identidade da entrada.** O par (seção, rótulo) identifica a entrada no registro, e o id do
  comando do console é derivado dele (ver ["O id do comando"](#o-id-do-comando)). Um rótulo que mudasse com o
  idioma do sistema mudaria o comando junto, e um passo a passo de report de bug ("rode
  `sistema.pausar_jogo`") passaria a depender da máquina de quem lê.
- **O menu se monta em runtime.** Não existe tela no editor onde alguém trocaria o texto por uma chave: as
  entradas nascem no call site que registrou. Exigir `tr()` obrigaria todo sistema de jogo a criar chave de
  localização para pendurar um botão de debug.

O que **não** é exceção: texto que a ferramenta faça aparecer no jogo de verdade. Se uma ação de debug
escrever numa Label do HUD, essa Label continua sob a regra do Guideline.
...
- Se for um toggle, o `on_changed` só aplica o estado, e quem lê o estado "de verdade" continua sendo o
  sistema, com o menu guardando só a cópia exibida?
...
- O texto novo é da ferramenta (fica hardcoded, ver ["Os textos da ferramenta não passam por
  `tr()`"](#os-textos-da-ferramenta-não-passam-por-tr)) e não texto que o jogador vai ler no jogo?
