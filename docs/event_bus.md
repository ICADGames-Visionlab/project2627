...
`scenes/dev/EventBusPlayground.tscn` mostra o padrão funcionando: três fontes de produção, relógio de mundo,
inventário, missões, carteira, áudio e HUD, com botões para iniciar produção, coletar, avançar dia e vender.
É o melhor lugar para ver o overlay em ação — inclusive o botão **Salvar**, que emite um pedido sem dono de
propósito, só para você ver o alerta acontecendo.

Os seis eventos que existem hoje foram escolhidos porque cada um ensina uma coisa:

| Evento | O que mostra |
| --- | --- |
| `production_collected` | Dois sistemas reagindo ao mesmo fato sem se conhecerem, com payload em classe |
| `day_started` | Três ouvintes de módulos diferentes no mesmo evento |
| `night_started` | Evento sem payload: o fato basta |
| `item_added` | A cadeia de dois saltos (`production_collected → item_added`), que é o limite |
| `gold_changed` | Valor novo + delta |
| `save_requested` | Pedido sem dono: o contra-exemplo |
...
Ele monta a mesma cena em memória, dirige o fluxo e confere que os efeitos aconteceram pelo bus. Sai com
código 1 se algo quebrar.
